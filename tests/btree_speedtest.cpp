...
struct btree_traits_speed : reservoir::btree_default_traits<size_t, size_t> {
...
struct TestFactory_Set {
    ...
    //! Test the forked B+ tree with a specific leaf/inner slots
    template <int Slots>
    struct BtreeSet
        : TestClass<reservoir::btree_multiset<
              size_t, std::less<>, struct btree_traits_speed<Slots, Slots>>> {
        BtreeSet(size_t n)
            : TestClass<reservoir::btree_multiset<
                  size_t, std::less<>, struct btree_traits_speed<Slots, Slots>>>(n) {
        }
    };

    //! Test the original tlx B+ tree with a specific leaf/inner slots
    template <int Slots>
    struct tlxBtreeSet
        : TestClass<tlx::btree_multiset<size_t, std::less<>,
                                        struct btree_traits_speed<Slots, Slots>>> {
        tlxBtreeSet(size_t n)
...
struct TestFactory_Map {
    ...
    template <int Slots>
    struct BtreeMap
        : TestClass<reservoir::btree_multimap<size_t, size_t, std::less<>,
                                              struct btree_traits_speed<Slots, Slots>>> {
        BtreeMap(size_t n)
            : TestClass<reservoir::btree_multimap<size_t, size_t, std::less<>,
                                                  struct btree_traits_speed<Slots, Slots>>>(
    ...
    template <int Slots>
    struct tlxBtreeMap
        : TestClass<tlx::btree_multimap<size_t, size_t, std::less<>,
                                        struct btree_traits_speed<Slots, Slots>>> {
        tlxBtreeMap(size_t n)
            : TestClass<tlx::btree_multimap<size_t, size_t, std::less<>,
                                            struct btree_traits_speed<Slots, Slots>>>(
                  n) {}
    };
...
void testrunner_loop(size_t items, const std::string& container_name) {
    ...
    std::cout << "RESULT"
              << " container=" << container_name << " op=" << TestClass::op()
              << " items=" << items << " repeat=" << repeat
              << " time_total=" << (ts2 - ts1) << " time=" << std::fixed
              << std::setprecision(10) << ((ts2 - ts1) / repeat)
              << " items_per_sec=" << items / (ts2 - ts1) << std::endl;
}
...
void TestFactory_Set<TestClass>::call_testrunner(size_t items) {
    ...
    btree_range<BtreeSet, min_nodeslots, max_nodeslots>()(
        items, "reservoir::btree_multiset");
#else
    // just pick a few node sizes for quicker tests
    testrunner_loop<BtreeSet<4>>(items, "reservoir::btree_multiset<4> slots=4");
    testrunner_loop<BtreeSet<8>>(items, "reservoir::btree_multiset<8> slots=8");
    testrunner_loop<BtreeSet<16>>(items,
                                  "reservoir::btree_multiset<16> slots=16");
    testrunner_loop<BtreeSet<32>>(items,
                                  "reservoir::btree_multiset<32> slots=32");
    testrunner_loop<BtreeSet<64>>(items,
                                  "reservoir::btree_multiset<64> slots=64");
    testrunner_loop<BtreeSet<128>>(items,
                                   "reservoir::btree_multiset<128> slots=128");
    testrunner_loop<BtreeSet<256>>(items,
                                   "reservoir::btree_multiset<256> slots=256");

    testrunner_loop<tlxBtreeSet<4>>(items, "tlx::btree_multiset<4> slots=4");
    testrunner_loop<tlxBtreeSet<8>>(items, "tlx::btree_multiset<8> slots=8");
    testrunner_loop<tlxBtreeSet<16>>(items, "tlx::btree_multiset<16> slots=16");
    testrunner_loop<tlxBtreeSet<32>>(items, "tlx::btree_multiset<32> slots=32");
    testrunner_loop<tlxBtreeSet<64>>(items, "tlx::btree_multiset<64> slots=64");
    testrunner_loop<tlxBtreeSet<128>>(items,
                                      "tlx::btree_multiset<128> slots=128");
    testrunner_loop<tlxBtreeSet<256>>(items,
...
void TestFactory_Map<TestClass>::call_testrunner(size_t items) {
    ...
    btree_range<BtreeMap, min_nodeslots, max_nodeslots>()(
        items, "reservoir::btree_multimap");
#else
    // just pick a few node sizes for quicker tests
    testrunner_loop<BtreeMap<4>>(items, "reservoir::btree_multimap<4> slots=4");
    testrunner_loop<BtreeMap<8>>(items, "reservoir::btree_multimap<8> slots=8");
    testrunner_loop<BtreeMap<16>>(items,
                                  "reservoir::btree_multimap<16> slots=16");
    testrunner_loop<BtreeMap<32>>(items,
                                  "reservoir::btree_multimap<32> slots=32");
    testrunner_loop<BtreeMap<64>>(items,
                                  "reservoir::btree_multimap<64> slots=64");
    testrunner_loop<BtreeMap<128>>(items,
                                   "reservoir::btree_multimap<128> slots=128");
    testrunner_loop<BtreeMap<256>>(items,
                                   "reservoir::btree_multimap<256> slots=256");


    testrunner_loop<tlxBtreeMap<4>>(items, "tlx::btree_multimap<4> slots=4");
    testrunner_loop<tlxBtreeMap<8>>(items, "tlx::btree_multimap<8> slots=8");
    testrunner_loop<tlxBtreeMap<16>>(items, "tlx::btree_multimap<16> slots=16");
    testrunner_loop<tlxBtreeMap<32>>(items, "tlx::btree_multimap<32> slots=32");
    testrunner_loop<tlxBtreeMap<64>>(items, "tlx::btree_multimap<64> slots=64");
    testrunner_loop<tlxBtreeMap<128>>(items,
                                      "tlx::btree_multimap<128> slots=128");
    testrunner_loop<tlxBtreeMap<256>>(items,
