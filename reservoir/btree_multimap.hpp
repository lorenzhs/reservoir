...
#pragma once
#ifndef RESERVOIR_BTREE_MULTIMAP_HEADER
#define RESERVOIR_BTREE_MULTIMAP_HEADER
...
public:
    ...
    using btree_impl =
        BTree<key_type, value_type, key_of_value, key_compare, traits, true, allocator_type>;
...
private:
    ...
    //! private constructor to wrap the btree_impl in a multimap
    btree_multimap(btree_impl&& other) noexcept : tree_(std::move(other)) {}
...
public:
    ...
    //! comparison function
    explicit btree_multimap(const allocator_type& alloc = allocator_type()) noexcept
        : tree_(alloc) {}
    ...
    friend void swap(btree_multimap& a, btree_multimap& b) noexcept {
        a.swap(b);
    }
...
public:
    ...
    bool operator==(const btree_multimap& other) const noexcept {
...
public:
    ...
    btree_multimap& operator=(const btree_multimap& other) noexcept {
        // no need to check for pointer equality as btree operator= already does
        tree_ = other.tree_;
        return *this;
    }

    //! Move assignment operator. No key/data pairs are copied or moved.
    btree_multimap& operator=(btree_multimap&& other) noexcept {
        // no need to check for pointer equality as btree operator= already does
    ...
    btree_multimap(const btree_multimap& other) noexcept : tree_(other.tree_) {}

    //! Move constructor. The newly initialized B+ tree object will contain all
    //! key/data pairs.
    btree_multimap(btree_multimap&& other) noexcept
        : tree_(std::move(other.tree_)) {}
...
public:
    ...
    //! never fails.
    iterator insert2(const key_type& key, const data_type& data) noexcept {
...
public:
    ...
    //! Return the rank of the leftmost element with the given key in O(log
    //! size()) or size() if there is no such element
    ...
    btree_multimap bulk_delete(size_type k) noexcept {
        return tree_.bulk_delete(k);
    }

    //! Delete the k smallest elements using split(). iter must be begin() + k.
    btree_multimap bulk_delete(size_type k, iterator iter) {
...
#endif // !RESERVOIR_BTREE_MULTIMAP_HEADER
