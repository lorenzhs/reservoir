...
#pragma once
#ifndef RESERVOIR_GENERATORS_MKL_HEADER
#define RESERVOIR_GENERATORS_MKL_HEADER
...
#endif
    void
    generate_block(double *ptr, size_t size, bool left_open = false) {
        check_size(size);

        MKL_INT count = static_cast<MKL_INT>(size);
        int status = vdRngUniform(VSL_RNG_METHOD_UNIFORM_STD, stream, count,
                                  ptr, 0.0, 1.0);
...
private:
    ...
    bool check_size(const size_t size) const {
...
#endif // RESERVOIR_HAVE_MKL

#endif // RESERVOIR_GENERATORS_MKL_HEADER
