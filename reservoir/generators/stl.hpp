...
#pragma once
#ifndef RESERVOIR_GENERATORS_STL_HEADER
#define RESERVOIR_GENERATORS_STL_HEADER
...
public:
    ...
    stl(const stl &other) : generator_(other.generator_) {}
    ...
    //! Generate a double from log(uniform (0,1])
    double next_log() {
        return std::log(next_left_open());
    }

    //! Generate a uniform double from (0, 1]
    double next_left_open() {
        return 1.0 - uniform_double_(generator_);
    }
    ...
    //! Generate `size` doubles from log(uniform(0, 1])
    void generate_log_block(std::vector<double> &output, size_t size) {
        if (size > output.size()) {
            output.resize(size);
        }
        generate_log_block(output.data(), size);
    }

    //! Generate `size` doubles from log(uniform(0, 1])
    void generate_log_block(double *arr, size_t size) {
        for (size_t i = 0; i < size; ++i) {
            arr[i] = std::log(1.0 - next());
        }
    }
...
#endif // RESERVOIR_GENERATORS_STL_HEADER
