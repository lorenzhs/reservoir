...
#pragma once
#ifndef RESERVOIR_BTREE_MULTISET_HEADER
#define RESERVOIR_BTREE_MULTISET_HEADER
...
private:
    ...
    //! private constructor to wrap the btree_impl in a multiset
    btree_multiset(btree_impl&& other) noexcept : tree_(std::move(other)) {}
...
public:
    ...
    //! comparison function
    explicit btree_multiset(const allocator_type& alloc = allocator_type()) noexcept
        : tree_(alloc) {}
    ...
    friend void swap(btree_multiset& a, btree_multiset& b) noexcept {
        a.swap(b);
    }
...
public:
    ...
    bool operator==(const btree_multiset& other) const noexcept {
...
public:
    ...
    btree_multiset& operator=(const btree_multiset& other) noexcept {
    ...
    btree_multiset& operator=(btree_multiset&& other) noexcept {
        // no need to check for pointer equality as btree operator= already does
    ...
    //! copy of all keys.
    btree_multiset(const btree_multiset& other) noexcept : tree_(other.tree_) {}

    //! Move constructor. The newly initialized B+ tree object will contain all
    //! keys.
    btree_multiset(btree_multiset&& other) noexcept
        : tree_(std::move(other.tree_)) {}
...
public:
    ...
    void join(btree_multiset& other_tree) noexcept {
    ...
    void splitAt(btree_multiset& left, size_type k, btree_multiset& right) noexcept {
        tree_.splitAt(left.tree_, k, right.tree_);
    }
    //! Split the set such that left has size k. iter must be begin() + k.
    void splitAt(btree_multiset& left, size_type k, iterator iter,
                 btree_multiset& right) noexcept {
        tree_.splitAt(left.tree_, k, iter, right.tree_);
    }

    //! Split the set such that every element in right is strictly greater than
    //! key
    void split(btree_multiset& left, key_type key, btree_multiset& right) noexcept {
    ...
    btree_multiset bulk_delete(size_type k) noexcept {
        return tree_.bulk_delete(k);
    }

    //! Delete the k smallest elements using split(). iter must be begin() + k.
    btree_multiset bulk_delete(size_type k, iterator iter) {
...
#endif // !RESERVOIR_BTREE_MULTISET_HEADER
